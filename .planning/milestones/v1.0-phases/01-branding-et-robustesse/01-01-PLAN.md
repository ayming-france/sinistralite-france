...
must_haves:
  ...
  key_links:
    ...
    - from: "js/nav.js"
      to: "landing.html"
      via: "shared localStorage key 'sinistralite-theme'"
      pattern: "sinistralite-theme"
---
...
The goal is zero references to 'DM Sans' in any JS file and all French text using proper accents.
  ...
  <verify>
    <automated>cd /Users/encarv/.claude/sinistralite && grep -rn "DM Sans" js/ ; echo "EXIT:$?"</automated>
...
Per user decision: "Same SVG icon reused in nav (scaled up) and browser tab (scaled down). One source, two sizes."
  ...
  <verify>
    <automated>cd /Users/encarv/.claude/sinistralite && grep -c "image/svg+xml" index.html && grep -c "image/svg+xml" landing.html</automated>
...
<task type="auto">
  ...
  <action>
1. In `js/nav.js`: Replace all occurrences of `localStorage.setItem('theme',` and `localStorage.getItem('theme')` with `localStorage.setItem('sinistralite-theme',` and `localStorage.getItem('sinistralite-theme')`. There are 3 occurrences: lines 21 (setItem light), 24 (setItem dark), and 84 (getItem).

The landing page already uses `'sinistralite-theme'` (landing.html lines 1801, 1804), so only the dashboard side needs updating.

2. Verify `landing.html` already uses `'sinistralite-theme'` (no change needed there for this task).

This ensures that toggling dark mode on either page carries over to the other.
  </action>
  <verify>
    <automated>cd /Users/encarv/.claude/sinistralite && grep -n "localStorage.*theme" js/nav.js | grep -v "sinistralite-theme" ; echo "EXIT:$?"</automated>
    <manual>Toggle dark mode on dashboard, navigate to landing page, verify dark mode is active</manual>
  </verify>
  <done>Dashboard uses 'sinistralite-theme' localStorage key. Theme choice persists between dashboard and landing page.</done>
</task>
...
4. `grep -n "localStorage.*'theme'" js/nav.js` returns zero matches (all use 'sinistralite-theme')
...
- Theme preference is shared between dashboard and landing via 'sinistralite-theme'
</success_criteria>
