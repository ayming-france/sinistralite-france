...
Simplest: show skeleton inside container, hide it after load. The header and views are already there but empty (no data rendered yet), so they won't show content until render() is called.
  ...
  <verify>
    <automated>cd /Users/encarv/.claude/sinistralite && grep -c "shimmer" styles/base.css && grep -c "loadingSkeleton" index.html && grep -c "errorState" index.html && grep -c "resp.ok\|response.ok" js/data.js</automated>
...
After removing, grep the entire project for each removed class name to confirm no orphaned references.
  ...
  <verify>
    <automated>cd /Users/encarv/.claude/sinistralite && for cls in empty-lead naf-table naf-row naf-badge naf-desc naf-example naf-label evo-delta tt-pct; do echo -n "$cls: "; grep -rn "$cls" styles/ js/ index.html 2>/dev/null | wc -l; done</automated>
